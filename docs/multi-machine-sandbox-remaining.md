...
## next-action
A–D are all DONE. What remains is hardening, not features: (1) a manual end-to-end run of the full flow (import → agent works → second machine attaches read-only → take-over → drop the link → auto-reconnect) against a real `docker run` sandbox over a real network (see unverified); (2) known caveats worth revisiting — clientId is `os.hostname()` (two machines with identical hostnames would fight over the drive; a persisted per-install UUID would fix it), and the sidebar/`git:stats` polling still targets the retired local `worktreePath` for sandbox-hosted workspaces (harmless log noise, no badges).

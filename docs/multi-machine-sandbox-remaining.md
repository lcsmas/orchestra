...
## next-action
(C) cross-machine ownership lock is the last remaining item; it needs the coordination-channel open-decision above resolved with the user first. Also worth doing: a manual end-to-end run of the full flow (import → agent → drop the link → auto-reconnect) against a real `docker run` sandbox over a real network (see unverified).

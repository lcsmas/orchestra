...
// on the worktree actually being the Orchestra codebase, so it is installed
// unconditionally like every other hook and stays silent everywhere else.
...
// two non-obvious footguns — edits only land after a release+install, and the
// worktree files Orchestra generates (hooks, skills, this very script) get
// rewritten on every spawn — so this SessionStart notice spells them out at
// the exact moment an agent starts working on the Orchestra repo. Double
// gate (package name AND the codebase-map dir) so a user's unrelated project
// that happens to be named "orchestra" never sees it.
const SELF_MODIFY_INSTRUCTION_SCRIPT = `#!/usr/bin/env bash
# Auto-installed by orchestra. Tells an agent whose worktree IS the Orchestra
# repo that it is modifying the app currently running it. Self-silences in
# every other repo.
root="\${ORCHESTRA_WORKTREE:-.}"
grep -q '"name": *"orchestra"' "\$root/package.json" 2>/dev/null || exit 0
[ -d "\$root/docs/codebase-map" ] || exit 0
cat <<'EOF'
[orchestra] Self-modification notice: this repo is Orchestra ITSELF — the app currently running you. Keep in mind:
- You are editing a git worktree; the app you run under is a separately installed build. Nothing you change takes effect until it is built, released, and installed (the repo's ship skill / scripts/release.sh drives that) — so editing is safe, and "my change does nothing" usually means "not shipped yet", not "broken".
...
export async function installOrchestraHooks(
  ...
  try {
    ...
    // workspace, so re-writing every script file + merging settings.json on every
